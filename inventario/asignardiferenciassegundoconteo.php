		    ...
		    if (isset($_SESSION['loggedin']))
				{	
					...
					$result = $pdo->prepare('select p.código as codigo ,p.nombre as nombre, c.* 
									   ...
									   where C.ConteoID =:CONTEO and con.Estado=:estado and con.Conteo3=:cont3 and c.usuasignado is null order by p.nombre');		 
					$result->bindParam(':CONTEO',$conteo,PDO::PARAM_STR);	
					$result->bindParam(':estado',$estado,PDO::PARAM_STR);	
					$result->bindParam(':cont3',$cont3,PDO::PARAM_STR);	
					$result->execute();
					$arreglo = array(); 
					$x=0; 
...
<div>
	<form name = "forminven" action="asignardiferenciassegundoconteo1.php" method="POST" >
		<table align= "center" >
			<tr> 
			</tr>
			<tr>
...
<?php
				...
				$acc = '1'; 
				$pdo = new PDO("sqlsrv:server=$sql_serverName ; Database = $sql_database", $sql_user, $sql_pwd);
				$result = $pdo->prepare('select usrid, usuario from seriesusr where acceso =:acc and anulado = 0 order by usrid desc');	
