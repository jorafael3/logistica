	...
	if (isset($_SESSION['loggedin']))
		{
			...
			$result = $pdo->prepare('INV_PRODUCTOS_DIFERENCIAS_CONTEO3 @CONTEO=:CONTEO , @SECCION=:SECCION, @BODEGA=:BODEGA');		 
			...
			while ( $y <= $count-1 ) 
				{
					...
					if ($diferencia<>0 ) 
						{
							$col = $col+1; 
							//cho "veces a insertar ". $y .$arreglo[$y][3].$diferencia.$seccion.$conteo;
...
//die(); 				
			...
			$estado = "Finalizado";
			$estconteo3 = "Finalizado"; 
			$pdo3 = new PDO("sqlsrv:server=$sql_serverName ; Database = $sql_database", $sql_user, $sql_pwd);
			$result3 = $pdo3->prepare('update inv_conteo set Estado=:estado ,  BodegaID=:bodega, Conteo3=:estconteo3
...
<div  id = "Cuerpo" >
	<div id="cuerpo2" >
					...
					<div id = "izq" >  </div>
					<div id = "centro" > <a class="titulo"> <center>   Finalizar Inventario  <?php echo $count2." Codigos" ?> </center> </a></div>
