	...
	if (isset($_SESSION['loggedin']))
		{
			...
			$result = $pdo->prepare('INV_PRODUCTOS_DIFERENCIAS @CONTEO=:CONTEO , @SECCION=:SECCION, @BODEGA=:BODEGA');		 
			...
			while ( $y <= $count-1 ) 
				{
					...
					if ($diferencia<>0 ) 
						{
							$col = $col+1; 
							//echo "veces a insertar ". $y .$arreglo[$y][3].$diferencia.$seccion.$conteo;
							$pdo1 = new PDO("sqlsrv:server=$sql_serverName ; Database = $sql_database", $sql_user, $sql_pwd);
							$result1 = $pdo1->prepare('INSERT INTO INV_CONTEO_DIFERENCIAS(ProductoID , Diferencia, Seccion, ConteoID, col, stockinventario ) 
													   VALUES (:ProductoID, :Diferencia, :Seccion, :ConteoID , :col, :stockinventario )');
							...
							$result1->bindParam(':stockinventario',$arreglo[$y][4],PDO::PARAM_STR);
							$result1->execute();
						}
				$y=$y+1;
				$diferencia= 0 ; 
...
//die(); 				
			...
			$result2 = $pdo2->prepare('select p.código as codigo ,p.nombre as nombre, c.* from  INV_CONTEO_DIFERENCIAS C
									   ...
									   where C.ConteoID =:CONTEO order by c.col');		 
			...
			$estado = "Asignacion";
			$estconteo1 = "Finalizado"; 
			$estconteo2 = "En Curso"; 
			$pdo3 = new PDO("sqlsrv:server=$sql_serverName ; Database = $sql_database", $sql_user, $sql_pwd);
			$result3 = $pdo3->prepare('update inv_conteo set Estado=:estado , Conteo1=:estconteo1 , BodegaID=:bodega, Conteo2=:estconteo2
									   where  ConteoID =:CONTEO');		 
			$result3->bindParam(':estado',$estado,PDO::PARAM_STR);	
			$result3->bindParam(':estconteo1',$estconteo1,PDO::PARAM_STR);	
