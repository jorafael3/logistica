...
<body>
	...
	<div id="Cuerpo">
		...
		<div id="cuerpo2" width="100%">
			...
			$sqlcas = "SELECT a.*, p.bodega as bodegaret, 
			...
			if ($sucufact == "") {
				...
				SELECT s.Nombre as TIENDA_RETIRO_NOMBRE, * from Cli_Direccion_Dropshipping c
				left join SIS_SUCURSALES s
				on c.tienda_retiro = s.ID
				where c.Facturaid = @factura
				");
				...
				if ($result3->execute()) {
					...
					if (count($res) > 0) {
						...
						$ciudad = "";
						$comentario = "";
						$direccion = $DATOS_DROP["Direccion"];
						$referencia = $DATOS_DROP["Referencia"];
						$celular = $DATOS_DROP["telefono"];
						$mail = $DATOS_DROP["Email"];
						$bodegaret = $DATOS_DROP["TIENDA_RETIRO_NOMBRE"];
					}
					// echo "<pre>";
					// var_dump($res3);
					// echo "</pre>";
			...
			if ($result7->execute()) {
				...
				if ($ESTA != "VERIFICADA") {
			?>
					<h1>ESTA FACTURA YA TIENE GUIA INGRESADA</h1>
			<?php
					die();
				}
				// echo $ESTA;
