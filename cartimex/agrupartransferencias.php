	...
	<div id="Cuerpo">
		<div id="cuerpo2">
			...
			<div id="centro"> <a class="titulo">
					<center> Agrupar Transferencias <?php echo substr($nomsuc, 10, 20);  ?> </center>
				</a></div>
					...
					<table align="center">
						...
						?>
							<tr>
								...
								<td>
									<button onclick="REVERSAR(this)">REVERSAR</button>
								</td>
								<td hidden id="ID"> <?php echo $arreglo[$y][1] ?></td>
...
<script>
	function REVERSAR(button) {
		...
		Swal.fire({
			title: "Se marcara como reversada?",
			showDenyButton: true,
	...
	function AjaxSend(param, callback) {
		...
		$.ajax({
			data: param,
			datatype: 'json',
			url: 'agurpartransferencias_f.php',
			type: 'POST',
			success: function(x) {
				x = JSON.parse(x)
				UnFreezeUI();
