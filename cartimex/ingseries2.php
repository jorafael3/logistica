...
<script type="text/javascript">
	...
	document.getElementById('cargar-archivo-button').addEventListener('click', function() {
		document.querySelector('#archivo-input').click();
	});

	document.querySelector('#archivo-input').addEventListener('change', function(event) {
		...
		if (archivo) {
			...
			reader.onload = function(e) {
				...
				lineas.forEach(function(linea, index) {
					const serieInput = document.querySelectorAll('input[name="serie"]')[index];
					if (serieInput) {
						serieInput.value = linea.trim();
					}
				});
			};
...
<body onload="setfocus()">
	...
	<div id="Cuerpo">
		...
		<hr>
		<div id="cuerpo2">
			<form name="form2" action="ingseries2.php" method="POST">
				<p style="font-weight: bold" align="center">Serie :
					<input name="serie" type="text" <?php echo $bloq ?> id="serie" size="30" value="<?php $serieleida ?>" style="padding: 5px; border: 1px solid #ccc; border-radius: 5px;">
					<input name="productoid" type="hidden" id="productoid" value="<?php echo $idproducto ?>">
					<input type="submit" name="submit" id="submit" value="-->" style="padding: 8px 15px; background-color: #3498db; border: none; border-radius: 5px; color: white; font-weight: bold; cursor: pointer;">
				</p>
			</form>
		</div>
		...
		<div id="cuerpo2" align="center">
			<table id="series" align="center">
				<form name="form2" action="ingseries1.php" method="POST">
					<?php
					$zz = 0;
					$label = 1;
					while ($zz <= $xx) {
					...
					?>
						<p style="font-weight: bold" align="center"> <?php echo $label ?>:
							<input name="serie" type="text" id="serie" size="30" value="<?php echo $series[1][$zz] ?>" style="padding: 5px; border: 1px solid #ccc; border-radius: 5px;">
