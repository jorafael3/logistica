...
$result = $pdo->prepare("select l.id, Usuario= u.nombre, Fecha= CONVERT( char(10), l.FFinRecep, 103),
						l.Detalle , rl.Contenedor, rl.Tipo, rl.Sello, rl.Sello2,rl.Placa , rl.Estiba,
						l.FInicioRecep, l.FFinRecep,rl.Nota, rl.Nota1, rl.Foto1, rl.Foto2, rl.Foto3, rl.Foto4, rl.Foto5, rl.Foto6, rl.Foto7, rl.Foto8
						from IMP_LIQUIDACION  L
						inner join IMP_RECEPCION_LIQ_CONTENEDOR rl on rl.LiquidacionID= l.ID
						inner join SERIESUSR u on u.usuario = l.RecibidoPor
						where l.id=:IDLiq " );
...
//Detalle de LIQUIDACIONES
    ...
    $result1 = $pdo1->prepare('select p.Código, p.Nombre, Facturado = sum(dt.facturado),  dt.CantRecibida, diferencia= dt.CantRecibida-sum(dt.facturado)
                      ...
                      order by 1');
    $result1->bindParam(':IDLiq',$liqid_clean,PDO::PARAM_STR);
    $result1->execute();
    $arreglo = array();
    $x=0;
		...
		if ($count==0)
		...
		else
			{
				...
				// PARA CAMBIAR ALEATORIAMENTE EL ENVIO DEL CORREO
								...
								$m->addAddress('vguijarro@cartimex.com');
								$m->addAddress('kvite@cartimex.com');
								$m->addAddress('mmacias@cartimex.com');
								$m->addAddress('jmacias@cartimex.com');
								$m->addAddress('rcorrea@cartimex.com');
								$m->isHTML(true);
								$m->AddAttachment($Foto1);
								$m->AddAttachment($Foto2);
								$m->AddAttachment($Foto3);
								$m->AddAttachment($Foto4);
								$m->AddAttachment($Foto5);
								$m->AddAttachment($Foto6);
								$m->AddAttachment($Foto7);
								$m->AddAttachment($Foto8);
