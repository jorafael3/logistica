...
if (isset($_SESSION['loggedin']))
	{
		    ...
		    $result = $pdo->prepare("select P.Código, P.NOMBRE, Facturado=  DT.cantrecibida
									from IMP_LIQUIDACION_DT DT
									INNER JOIN INV_PRODUCTOS P ON P.ID = DT.ProductoID
									WHERE DT.LiquidaciónID =:IDLiq AND ProductoID=:productoid
			...
			while ($row = $result->fetch(PDO::FETCH_ASSOC)) 
				{
					$Codigo=$row['Código'];
					$Producto=$row['NOMBRE'];
					$Cantidad=number_format($row['Facturado'], 0, '.', '');
...
<div  id = "Cuerpo" >
	<div id="cuerpo2" >
		<div id = "izq" > </div>
		<div id = "centro" > <a class="titulo"> <center>   IMPORTACIONES  </center> </a></div>
	...
	<div id="cuerpo2"  >
		<div align= "left" >
			...
			<table  align= "center" border="2" cellpadding="5" cellspacing="1">
				<tr>
					<form method="POST" action="registraseriesimportaciones.php" enctype="multipart/form-data" id="myform">
						<tr>
						...
						<tr>
							...
							<input type="hidden" name="LiqID" value="<?php echo $IDLiq ?>" />
...
<div id= "cuerpo2" align= "center" >
	<div class=\"table-responsive-xs\">
		<form name = " " action="registraseriesimportaciones1.php" method="POST" width="100%">
			<table  align= "center"  >
					<tr>
						<td id= "label">  
...
if ($manual==0)
...
else
	{
		...
		$result = $pdo->prepare("select serie from inv_productos_series_compras
									WHERE  LiquidacionID =:IDLiq AND ProductoID=:productoid" );
		$result->bindParam(':IDLiq',$IDLiq,PDO::PARAM_STR);
		$result->bindParam(':productoid',$productoid,PDO::PARAM_STR);					
		$result->execute();
		$arreglo = array();
		...
		while ( $y <= $Cantidad-1 ) 
			{
				echo $$arreglo[$y][1]; 
				echo "<tr><td>".$n ."</td> <td  align=left >" . $Codigo . "</td><td id= box > <input name=serie[] type=text id=serie[] required value=".$arreglo[$y][1]. "  > </td> </tr>";
