		...
		if (isset($_SESSION['loggedin']))
			{
					...
					$resultdt = $pdodt->prepare('select P.Código, P.Nombre, LDT.Ingresado,
												p.registroSeries, Ingresadas= COUNT(SC.SERIE) , ProductoID= p.id, ldt.PedidoID ,ldt.id
												...
												LEFT OUTER JOIN INV_PRODUCTOS_SERIES_COMPRAS SC WITH (NOLOCK) ON L.ID = SC.LiquidacionID and sc.productoid= LDT.ProductoID
												where L.ID =:ID
												GROUP BY LDT.ProductoID,LDT.Ingresado, P.Código, P.Nombre, p.registroSeries, p.id , ldt.PedidoID  ,ldt.id	 order by 1');
