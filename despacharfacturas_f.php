	...
	try {
		...
		foreach ($DATOS as $row) {
			$result = $pdo->prepare("Log_facturaslistas_despachar_update 
				...
				@usuario=:usuario,
				@bodegaFAC=:bodegaFAC");
			$result->bindParam(':numfac', $row["secuencia"], PDO::PARAM_STR);
			$result->bindParam(':usuario', $usuario, PDO::PARAM_STR);
			$result->bindParam(':bodegaFAC', $row["BodegaFAC"], PDO::PARAM_STR);
			$result->execute();
...
function Correo($DATOS)
{
	...
	try {
		...
		$rastreo = '';
		$Transporte = '';
		$Transporte = $SISCO['despachofinal'];
		$guia = $SISCO['despacho'];
		$sucursal = $SISCO['sucursal'];
